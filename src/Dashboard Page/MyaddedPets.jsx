...
const MyAddedPets = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "image",
        Cell: ({ value }) => (
          <img src={value} alt="Pet" className="rounded-3xl h-20 w-16" />
        ),
      },
      {
          ...
          <div>
            ...
            {row.original.adopted ? (
              <>
                {" "}
                <button
                  onClick={() => toggleAdoptionStatusp(row.original._id)}
  ...
  // --------------------
  const toggleAdoptionStatusp = (id) => {
    fetch(`https://server-twelve.vercel.app/pet/adopti/${id}`, {
      method: "PUT",
    })
      .then((res) => res.json())
      .then(() => {
  ...
  return (
    <div className="bg-[#1e272e] min-h-screen">
      ...
      <div className="table overflow-x-auto px-4">
        <table
          {...getTableProps()}
          className="min-w-full bg-black text-white bg-opacity-50"
        >
          <thead>
        ...
        {pageOptions.length > 1 && (
          <div className="pagination flex items-center justify-center mb-5">
            <button
              ...
              className="text-white text-5xl rounded mx-1"
            >
              {"<<"}
            </button>
            <button
